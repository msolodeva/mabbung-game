...
class LevelGenerator:
    ...
    def spawn_chunk(self, start_x, width):
        ...
        if random.random() < 0.35:
            base = random.choice(chunk_plats)
            if base.width > 120:
                dx = random.randint(base.left + 20, base.right - 60)
                dy = base.top - 38
                roam_left = base.left + 10
                roam_right = base.right - 10
                dino = {
    ...
    def _generate_sea_biome(self, start_x, width, ground, chunk_plats):
        ...
        if ground.colliderect(sea):
            ...
            if sea.left > ground.left:
                left_ground = pygame.Rect(ground.left, ground.top, sea.left - ground.left, ground.height)
                self.entity_manager.platforms.append(left_ground)
                chunk_plats.append(left_ground)
            if ground.right > sea.right:
                right_ground = pygame.Rect(sea.right, ground.top, ground.right - sea.right, ground.height)
