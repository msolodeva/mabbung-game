...
class EntityManager:
    ...
    def reset_to_initial_state(self):
        ...
        self.moving_platforms = [
            {"rect": pygame.Rect(1350, 340, 120, 20), "vx": 2, "left": 1350, "right": 1650},
            {"rect": pygame.Rect(1850, 260, 100, 20), "vx": -2, "left": 1750, "right": 2050},
        ]
        
        self.vertical_platforms = [
            {"rect": pygame.Rect(1600, 260, 100, 20), "vy": 2, "top": 220, "bottom": 360},
        ]
        ...
        self.enemies = [
            ...
            {"rect": pygame.Rect(1720, 392, 28, 28), "vx": 2, "vy": 0, "alive": True, "kind": "hopper", "jump_cd": 60},
        ]
    ...
    def update_mushrooms(self):
        """버섯 아이템 업데이트"""
        all_platforms = self.platforms + [mp["rect"] for mp in self.moving_platforms] + [vp["rect"] for vp in self.vertical_platforms]
    ...
    def update_dinos(self):
        ...
        for d in self.dinos:
            ...
            if d["rider"]:
                # 탑승 중
                player_rect = d["rider"]
                d["rect"].centerx = player_rect.centerx
                d["rect"].bottom = player_rect.bottom + 6
                continue
    ...
    def cleanup_offscreen(self, despawn_x):
        ...
        self.moving_platforms = [mp for mp in self.moving_platforms if mp["rect"].right > despawn_x]
        self.vertical_platforms = [vp for vp in self.vertical_platforms if vp["rect"].right > despawn_x]
        self.fragile_platforms = [fp for fp in self.fragile_platforms if fp["rect"].right > despawn_x and fp["rect"].width > 0]
        self.enemies = [e for e in self.enemies if e["rect"].right > despawn_x and (e["alive"] or e["rect"].y < SCREEN_HEIGHT + 100)]
        ...
        self.mushrooms = [m for m in self.mushrooms if m["rect"].right > despawn_x and m["alive"]]
        self.seas = [w for w in self.seas if w.right > despawn_x]
        self.fish_enemies = [f for f in self.fish_enemies if f["rect"].right > despawn_x and f["alive"]]
        self.turtle_enemies = [t for t in self.turtle_enemies if t["rect"].right > despawn_x and t["alive"]]
        self.jellies = [j for j in self.jellies if j["rect"].right > despawn_x and j["alive"]]
